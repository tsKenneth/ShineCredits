...
BadgesTab = {}
BadgesTab.TabName = "Badges"
BadgesTab.BadgesListData = {}
BadgesTab.PurchasedBadges = {}
...
function BadgesTab.OnInit( Panel, Data )
    ...
    BadgesTab.PanelLabel:SetAnchor( "TopLeft" )
    ...
    BadgesTab.CreditBalanceLabel:SetText( "Available Credits: "
        .. BadgesTab.CurrentCredits)
    BadgesTab.CreditBalanceLabel:SetAnchor( "TopRight" )
    BadgesTab.CreditBalanceLabel:SetPos( Vector( -200, 25, 0 ) )
    ...
    BadgesTab.PreviewImage:SetAnchor( "TopRight" )
    BadgesTab.PreviewImage:SetSize( Vector( 150, 150, 0 ) )
    BadgesTab.PreviewImage:SetPos( Vector( -200, 100, 0  ) )

    -- Populate the Badges Menu
    BadgesTab.BadgesList = SGUI:Create( "List", Panel )
    BadgesTab.BadgesList:SetAnchor( "TopLeft" )
    BadgesTab.BadgesList:SetPos( Vector( 20, 100, 0 ) )
    BadgesTab.BadgesList:SetColumns( "Badge Name", "Cost" )
    BadgesTab.BadgesList:SetSpacing( 0.7, 0.3 )
    BadgesTab.BadgesList:SetSize( Vector( 400, 750, 0 ) )
    ...
    BadgesTab.RedemptionButton:SetAnchor( "BottomMiddle" )
    BadgesTab.RedemptionButton:SetSize( Vector( 150, 50, 0 ) )
    BadgesTab.RedemptionButton:SetPos( Vector( -75, -100, 0 ) )
    BadgesTab.RedemptionButton:SetText( "Redeem" )
    BadgesTab.RedemptionButton.DoClick = function( Button )
        local BadgeSelected = BadgesTab.BadgesList:GetSelectedRow()
        if BadgeSelected then
            Shared.ConsoleCommand(string.format(BadgesTab.RedeemCommand ..
                " \"%s\"",
                BadgeSelected:GetColumnText( 1 )))
        end
    end
